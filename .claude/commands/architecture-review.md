Perform an ARCHITECTURE analysis on PR: $ARGUMENTS
...
Read the specialist profile at `profiles/agents/architecture.md`.
...
Read local source files to understand the project's architectural patterns. Prioritize:
- **Project structure**: List the top-level directories (e.g., `ls src/`) to understand layering
- **DI registration**: Startup.cs, Program.cs, ServiceCollectionExtensions, module definitions
- **Interfaces and abstractions**: Interface definitions referenced by modified code
- **Base classes and shared infrastructure**: Repository base, service base, middleware pipeline
- **Project/package structure**: .csproj references, package.json workspaces, go.mod dependencies

Read up to 15 related files. If the repository is not cloned locally, skip this step and analyze the diff only.

### 5. Architecture Analysis
Analyze the diff from a structural and design perspective:

- **SOLID Principles**: Violations of Single Responsibility, Open/Closed, Liskov, Interface Segregation, Dependency Inversion
- **Layering**: Layer boundary violations, business logic leaking to wrong layers
- **Coupling/Cohesion**: Tight coupling, low cohesion, circular dependencies
- **API Design**: Contract consistency, DTO separation, versioning
- **Design Patterns**: Appropriate usage, anti-patterns (God class, Service locator, etc.)
- **Modularity**: Extension points, configuration externalization, dependency injection

### 6. Report Generation
Read the review language from `config.json > review.language` and generate the report in that language:

```markdown
## 🏗️ Architecture Review - [PR title]
...
[High-level assessment of the architectural impact of these changes]

### SOLID Analysis
[Any SOLID violations found]

### Layering and Boundaries
[Layer violations, boundary crossings]

### Coupling and Cohesion
[Coupling issues, cohesion concerns]

### Design Patterns
[Pattern usage, anti-patterns detected]
...
**Principle:** [SOLID principle or architecture concept violated]
...
**[SOUND / NEEDS_REFACTORING / RETHINK]** - [justification]
```
...
- `ICON` — `🏗️` (architecture review)
- `REVIEW_TYPE_LABEL` — localized from i18n (`labels.architecture_review`)
